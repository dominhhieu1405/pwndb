...
def main(emails, output=None):
    if not output:
        print(info + " Đang tìm dữ liệu bị rò rỉ...")
    ...
    if not results:
        if not output:
            print(bad + " Không có dữ liệu." + end)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--target", help="Nhắm mục tiêu email/miền để tìm kiếm rò rỉ")
    parser.add_argument("--list", help="Danh sách các email trong một tệp để tìm kiếm rò rỉ.")
    parser.add_argument("--output", help="Trả về kết quả dưới dạng json/txt")
    parser.add_argument("--proxy", default='127.0.0.1:9050', type=str, help="Đặt proxy Tor (Mặc định: 127.0.0.1:9050)")
    ...
    if args.output:
        if args.output not in ['json', 'txt']:
            print(bad + " Đầu ra phải là json hoặc txt" + end)
            exit(-1)
        output = args.output
    ...
    if args.list:
        try:
            ...
            print(bad + " Không thể đọc tệp: " + str(args.list))
            exit(-1)
    try:
        main(emails, output)
    except ConnectionError:
        print(bad + " Không thể kết nối với dịch vụ! Khởi động lại tor và thử lại.")
    except Exception as e:
        print(bad + " " + str(e))
